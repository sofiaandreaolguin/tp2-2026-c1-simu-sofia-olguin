...
export async function getAllMoviesController(req, res) {
 try {
        ...
        const genres = req.query.genre ? { genres: req.query.genre } : {};
        //capaz que directo req.query 
        const movies = await getAllMovies({ page, limit }, genres );
        res.json(movies);
    } catch (error) {
...
export async function getAwardWinnersController(req, res) {
    try{
        ...
        console.error("Error fetching winner movies: ", error);
        res.status(500).json({message: "Internal server error"});
    }
